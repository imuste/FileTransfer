...
{       
        ...
        writeRRQ(theInfo);
        readData1(theInfo);
        readData2(theInfo); // ignore this packet
        writeAck0(theInfo);
        sleep(3);
        readData3(theInfo); 
        readData4(theInfo); // this is the real packet 1
        writeAck1(theInfo);
        sleep(3);
        readData5(theInfo); 
        readData5(theInfo); // read the empty packet (EOF)
        writeAck2(theInfo);
...
void writeRRQ(info *theInfo)
{
        ...
        header1.msgType = 1;
        header1.winSeq = 3;
        strcpy(header1.data, "client6_GT.txt");
...
void readData1(info *theInfo)
{
        ...
        theInfo->fileContents = (char *)malloc(1024);
