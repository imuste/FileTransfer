...
void writeAcks(info *theInfo);
void readData2(info *theInfo);
void writeAcks2(info *theInfo);
...
void writeRRQ(info *theInfo)
{
        ...
        header1.msgType = 1;
        header1.winSeq = 3;
        strcpy(header1.data, "client1_GT.txt");
...
void readData(info *theInfo)
{
        ...
        theInfo->fileContents = (char *)malloc(2463);
...
void writeAcks(info *theInfo)
{
        // for (int i = 0; i < 3; i++) {
                ...
                int serverAddrLen = sizeof(theInfo->serverAddr);
                int returnVal = sendto(theInfo->clientSD, (void *)&header1, 
                        2, 0, (struct sockaddr *)&theInfo->serverAddr, 
                        serverAddrLen);
                assert(returnVal != -1);
        // }
